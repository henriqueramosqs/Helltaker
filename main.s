...
checaPedraCima1:
	...
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, -10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraCima1		# Se pedra estiver apoiada, n�o move
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraCima1		# Se pedra estiver apoiada, n�o move
	li t4, 'B'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraCima1		# Se pedra estiver apoiada, n�o move	
	sb t0, -10(t1)				# Se for pedra, muda a mem�ria do quadrado acima para P
...
naoMovePedraCima1:	
	addi a6, a6, 1						# Corrige a posi��o de volta para o personagem
	jal s11, animacaoChute
cimaLivre:
	jal calculaPosicaoFase2	
	la a0, hero
...
checaPedraEsq1:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, esquerdaLivre			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, -1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraEsq1		# Se pedra estiver apoiada, n�o move
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraEsq1		# Se pedra estiver apoiada, n�o move
	li t4, 'B'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraEsq1		# Se pedra estiver apoiada, n�o move	
	sb t0, -1(t1)				# Se for pedra, muda a mem�ria do quadrado acima para P
	li t0, '0'				# Carrega 0 que representa espa�o vazio
	sb t0, 0(t1)				# Muda a mem�ria no quadrado para espa�o vazio
...
naoMovePedraEsq1:	
	addi a3, a3, +1						# Corrige a posi��o de volta para o personagem	
	jal s11, animacaoChute
...
checaPedraBaixo1:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, baixoLivre			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, 10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraBaixo1		# Se pedra estiver apoiada, n�o move
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraBaixo1		# Se pedra estiver apoiada, n�o move
	li t4, 'B'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraBaixo1		# Se pedra estiver apoiada, n�o move	
...
checaPedraDir1:
	...
	lb t3, 1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraDir1		# Se pedra estiver apoiada, n�o move
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraDir1		# Se pedra estiver apoiada, n�o move
	li t4, 'B'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraDir1		# Se pedra estiver apoiada, n�o move	
...
# Salvando a colis�o em um novo local da mem�ria para n�o afetar a original
...
checaEsqueletoCima:
	...
	bne t2, t0, checaPedraCima		# Checa se tem esqueleto, se n�o segue normalmente
	...
	sb t0, -10(t1)				# Se for esqueleto, muda a mem�ria do quadrado acima para E
	li t0, '0'				# Carrega 0 que representa espa�o vazio
	sb t0, 0(t1)				# Muda a mem�ria no quadrado para espa�o vazio
	...
	addi a6, a6, -1						# Sobe uma posi��o na matriz
	la a4, esqueleto
	jal s9, spriteNotImm
	
	addi a6, a6, 2						# Corrige a posi��o de volta para o personagem
	jal s11, animacaoChute
	j checaEspinhoCima2
MorteDoEsqueleto:
	sb zero, (t1)				# Se for pra morrer, muda a mem�ria do quadrado do esqueleto pra 0
	la a4, tampao
	jal s9, spriteNotImm
	...
	mv s0, a3		# salva o valor de a3, porque ser� usado pra volume
...
checaPedraCima:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, checaEspinhoCima2			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, -10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraCima2		# Se pedra estiver apoiada, n�o move
	lb t3, -10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraCima2		# Se pedra estiver apoiada, n�o move
	sb t0, -10(t1)				# Se for pedra, muda a mem�ria do quadrado acima para P
...
checaEsqueletoEsq2:
	addi s10, s10, -1
	li t0, 'E'				# E representa esqueleto no mapa
	bne t2, t0, checaPedraEsq2		# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver esqueleto e quadrado acima do esqueleto for "x", O Esqueleto morre
	lb t3, -1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	...
	la a4, esqueleto
	jal s9, spriteNotImm	
	addi a3, a3, 2						# Corrige a posi��o de volta para o personagem
	jal s11, animacaoChute
	j checaEspinhoEsq2
...
checaPedraEsq2:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, checaEspinhoEsq2			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, -1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraEsq2		# Se pedra estiver apoiada, n�o move
	lb t3, -1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraEsq2		# Se pedra estiver apoiada, n�o move
	sb t0, -1(t1)				# Se for pedra, muda a mem�ria do quadrado acima para P
...
checaEsqueletoBaixo:
	addi s10, s10, -1
	li t0, 'E'				# E representa esqueleto no mapa
	bne t2, t0, checaPedraBaixo2		# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver esqueleto e quadrado acima do esqueleto for "x", O Esqueleto morre
	lb t3, 10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
...
MorteDoEsqueletoBx:
	sb zero, (t1)				# Se for pra morrer, muda a mem�ria do quadrado do esqueleto pra 0
	jal calculaPosicaoFase2					# Desenha o tamp�o onde estava o esqueleto	
...
checaPedraBaixo2:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, checaEspinhoBaixo2			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, 10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraBaixo2		# Se pedra estiver apoiada, n�o move
	lb t3, 10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraBaixo2		# Se pedra estiver apoiada, n�o move
	sb t0, 10(t1)				# Se for pedra, muda a mem�ria do quadrado acima para P
...
checaEsqueletoDireita2:
	addi s10, s10, -1
	li t0, 'E'				# E representa esqueleto no mapa
	bne t2, t0, checaPedraDir2		# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver esqueleto e quadrado acima do esqueleto for "x", O Esqueleto morre
	...
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, MorteDoEsqueletoDir2		# Se esweuelto estiver encurralado, ele morre
	sb t0, 1(t1)				# Se for esqueleto, muda a mem�ria do quadrado do lado para E
	li t0, '0'				# Carrega 0 que representa espa�o vazio
	sb t0, 0(t1)				# Muda a mem�ria no quadrado para espa�o vazio
...
checaPedraDir2:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, checaEspinhoDir2			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, 1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraDir2		# Se pedra estiver apoiada, n�o move
	lb t3, 1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraDir2		# Se pedra estiver apoiada, n�o move
	sb t0, 1(t1)				# Se for pedra, muda a mem�ria do quadrado acima para P
...
fase_2AbreDialogo:
# Sempre come�ar o di�logo na primeira op��o
	li t0, 0xFF200604
	li t1, 0
	sw t1, 0(t0)
...
checaEsqueletoCima3:
	addi s10,s10,-1
	li t0, 'E'				# E representa esqueleto no mapa
	bne t2, t0, checaPedraCima3		# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver esqueleto e quadrado acima do esqueleto for "x", O Esqueleto morre
	lb t3, -10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
...
checaPedraCima3:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, checaEspinhoCima3			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, -10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraCima3		# Se pedra estiver apoiada, n�o move
	lb t3, -10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraCima3		# Se pedra estiver apoiada, n�o move
	sb t0, -10(t1)				# Se for pedra, muda a mem�ria do quadrado acima para P
...
checaEsqueletoEsq3:
	addi s10, s10, -1
	li t0, 'E'				# E representa esqueleto no mapa
	bne t2, t0, checaPedraEsq3		# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver esqueleto e quadrado acima do esqueleto for "x", O Esqueleto morre
	lb t3, -1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
...
checaPedraEsq3:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, checaEspinhoEsq3			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, -1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraEsq3		# Se pedra estiver apoiada, n�o move
	lb t3, -1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraEsq3		# Se pedra estiver apoiada, n�o move
	sb t0, -1(t1)				# Se for pedra, muda a mem�ria do quadrado acima para P
...
checaEsqueletoBaixo3:
	addi s10, s10, -1
	li t0, 'E'				# E representa esqueleto no mapa
	bne t2, t0, checaPedraBaixo3		# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver esqueleto e quadrado acima do esqueleto for "x", O Esqueleto morre
	lb t3, 10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
...
MorteDoEsqueletoBx3:
	sb zero, (t1)				# Se for pra morrer, muda a mem�ria do quadrado do esqueleto pra 0
	jal calculaPosicaoFase3				# Desenha o tamp�o onde estava o esqueleto	
...
checaPedraBaixo3:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, checaEspinhoBaixo3			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, 10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraBaixo3		# Se pedra estiver apoiada, n�o move
	lb t3, 10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraBaixo3		# Se pedra estiver apoiada, n�o move
	sb t0, 10(t1)				# Se for pedra, muda a mem�ria do quadrado acima para P
...
checaEsqueletoDireita3:
	addi s10, s10, -1
	li t0, 'E'				# E representa esqueleto no mapa
	bne t2, t0, checaPedraDir3		# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver esqueleto e quadrado acima do esqueleto for "x", O Esqueleto morre
	lb t3, 1(t1)				#Armazena o quadrado a direita do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
...
checaPedraDir3:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, checaEspinhoDir3			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, 1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraDir3		# Se pedra estiver apoiada, n�o move
	lb t3, 1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraDir3	 # Se pedra estiver apoiada, n�o move
	sb t0, 1(t1)				# Se for pedra, muda a mem�ria do quadrado acima para P
...
checaEsqueletoCima4:
	addi s10,s10,-1
	li t0, 'E'				# E representa esqueleto no mapa
	bne t2, t0, checaPedraCima4		# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver esqueleto e quadrado acima do esqueleto for "x", O Esqueleto morre
	lb t3, -10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
...
checaPedraCima4:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, checaEspinhoCima4			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, -10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraCima4		# Se pedra estiver apoiada, n�o move
	lb t3, -10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraCima4		# Se pedra estiver apoiada, n�o move
	lb t3, -10(t1)	
...
checaBauCima4:	
	...
	bne t3, t0, cimaLivre4		        # Se o da direita n�o for B, anda 
	bne s5, zero, bauComChaveCima4          #Se tiver unm B e estiver com chave, abre o ba�	
	                        		#Se tiver ba� mas n tiver chave:	       
	addi a6,a6, 1			        #recalcula posicao do her�i e contador de passos 
	addi s10,s10,1
	la a4, hero
...
bauComChaveCima4:
	#animacaoBau				#Para m�ritos de teste
cimaLivre4:
	la a4, hero
	jal s9, spriteNotImm3
	j fase4_loop
...
checaEsqueletoEsq4:
	addi s10, s10, -1
	li t0, 'E'				# E representa esqueleto no mapa
	bne t2, t0, checaPedraEsq4    # Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver esqueleto e quadrado acima do esqueleto for "x", O Esqueleto morre
	lb t3, -1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
...
checaPedraEsq4:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, checaEspinhoEsq4			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, -1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraEsq4		# Se pedra estiver apoiada, n�o move
	lb t3, -1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraEsq4		# Se pedra estiver apoiada, n�o move
	lb t3, -1(t1)				#Armazena o quadrado acima do esqueleto
...
checaEsqueletoBaixo4:
	addi s10, s10, -1
	li t0, 'E'				# E representa esqueleto no mapa
	bne t2, t0, checaPedraBaixo4		# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver esqueleto e quadrado acima do esqueleto for "x", O Esqueleto morre
	lb t3, 10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
...
checaPedraBaixo4:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, checaEspinhoBaixo4			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, 10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraBaixo4		# Se pedra estiver apoiada, n�o move
	lb t3, 10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraBaixo4		# Se pedra estiver apoiada, n�o move
	lb t3, 10(t1)	
...
checaEspinhoBaixo4:
	...
	bne t2, t0, checaChaveBaixo4			# Checa se tem esqueleto, se n�o segue normalmente		#Se n�o for espinho, checa chave
	la t1, colisao_temporaria
	li t2, 10
	mul t2, a6, t2
	add t2, t2, a3
...
checaChaveBaixo4:
	...
	mul t2, a6, t2
	add t2, t2, a3
	add t1, t1, t2				#Atualiza matriz de colis�o tempor�ria,
	lb t2, 0(t1)
	#jal s11, animacaoChave			#Roda animac�o da chave
	#jal s11,animacaoFuro			#Para m�ritos de teste
...
checaEsqueletoDireita4:
	addi s10, s10, -1
	li t0, 'E'				# E representa esqueleto no mapa
	bne t2, t0, checaPedraDir4		# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver esqueleto e quadrado acima do esqueleto for "x", O Esqueleto morre
	lb t3, 1(t1)				#Armazena o quadrado a direita do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
...
checaPedraDir4:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, checaEspinhoDir4			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, 1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraDir4		# Se pedra estiver apoiada, n�o move
	lb t3, 1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraDir4  # Se pedra estiver apoiada, n�o move
	sb t0, 1(t1)				# Se for pedra, muda a mem�ria do quadrado acima para P
...
deixaEspinhoDir4:
	...
	lb t2, 0(t1)
	addi a3, a3, 1
	bne t2, t0, checaBauDireita4      #Se n�o for espinho, ele checa se � ba�
	addi a3, a3, -1
	la a4, espinho
	jal s9, spriteNotImm3
	addi a3, a3, 1
...
checaBauDireita4:	
	...
	li t0, 'B'				# Carrega "B"
	lb t3, 1(t1)				#Armazena o quadrado a direita do esqueleto
	bne t3, t0, DireitaLivre4		# Se o da direita n�o for B, anda 
	bne s5, zero, bauComChaveDireita4	 #Se tiver unm B e estiver com chave, abre o ba�	
						#Se tiver ba� mas n tiver chave:	       
	addi a3,a3, -1			        #recalcula posicao do her�i e contador de passos 
	addi s10,s10,1
	la a4, hero
	jal s9, spriteNotImm3			#desenha hero no "lugar certo"
	j fase4_loop		       	
...
plotaHeroi5:
	...
	li s6, 4			        # Marca o eixo y do ponto que abre a caixa de dialogo
...
fase5_loop:
	beq a3, s3, fase_5DialogCase
	ble s10, zero, fase5_morte
fase_5AfterComparison:
# Mostrando a vida na tela
	mv s9, a3
	mv a0, s10
	li t0, 10 
	
	blt a0, t0, digito_unico5
...
# Checando teclas pressionadas
	jal readKeyNonBlocking			# l? input do usu?rio	
	li t0, 'w'				# armazena c?digo da letra w em t0
	beq a0, t0 , moveCima5		# Se input for w, roda o comando de mover para cima
	li t0, 'a'
	beq a0, t0, moveEsquerda5
	li t0, 's'
	beq a0, t0, moveBaixo5
	li t0, 'd'
	beq a0, t0, moveDireita5
	j fase5_loop					# Se n?o for, checa o caso do input ser a
moveCima5:
	#addi s10, s10, -1
	la a4, tampao
	jal s9, spriteNotImm3
	...
	add t1, t1, t2
	lb t2, 0(t1)
	bne t2, t0, checaEsqueletoCima5
	addi a6, a6, 1
	j checaEspinhoCima5
checaEsqueletoCima5:
	addi s10,s10,-1
	li t0, 'E'				# E representa esqueleto no mapa
	bne t2, t0, checaPedraCima5	 # Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver esqueleto e quadrado acima do esqueleto for "x", O Esqueleto morre
	lb t3, -10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
...
checaPedraCima5:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, checaEspinhoCima5			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, -10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraCima5		# Se pedra estiver apoiada, n�o move
	lb t3, -10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraCima5		# Se pedra estiver apoiada, n�o move
	lb t3, -10(t1)	
...
checaBauCima5:	
	...
	li t0, 'B'				# Carrega "B"
	lb t3, -10(t1)				#Armazena o quadrado a direita do esqueleto
	bne t3, t0, cimaLivre5		        # Se o da direita n�o for B, anda 
	bne s5, zero, bauComChaveCima5          #Se tiver unm B e estiver com chave, abre o ba�	
	                        		#Se tiver ba� mas n tiver chave:	       
	addi a6,a6, 1			        #recalcula posicao do her�i e contador de passos 
	addi s10,s10,1
	la a4, hero
...
moveEsquerda5:
	...
	add t1, t1, t2
	lb t2, 0(t1)
	bne t2, t0, checaEsqueletoEsq5
	addi a3, a3, 1
	j checaEspinhoEsq5
checaEsqueletoEsq5:
	addi s10, s10, -1
	li t0, 'E'				# E representa esqueleto no mapa
	bne t2, t0, checaPedraEsq5    # Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver esqueleto e quadrado acima do esqueleto for "x", O Esqueleto morre
	lb t3, -1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
...
checaPedraEsq5:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, checaEspinhoEsq5			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, -1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraEsq5		# Se pedra estiver apoiada, n�o move
	lb t3, -1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraEsq5		# Se pedra estiver apoiada, n�o move
	lb t3, -1(t1)				#Armazena o quadrado acima do esqueleto
...
moveBaixo5:
	...
	add t1, t1, t2
	lb t2, 0(t1)
	bne t2, t0, checaEsqueletoBaixo5
	addi a6, a6, -1
...
checaEsqueletoBaixo5:
	addi s10, s10, -1
	li t0, 'E'				# E representa esqueleto no mapa
	bne t2, t0, checaPedraBaixo5		# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver esqueleto e quadrado acima do esqueleto for "x", O Esqueleto morre
	lb t3, 10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
...
checaPedraBaixo5:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, checaEspinhoBaixo5			# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, 10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraBaixo5		# Se pedra estiver apoiada, n�o move
	lb t3, 10(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'P'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraBaixo5		# Se pedra estiver apoiada, n�o move
	lb t3, 10(t1)	
...
checaEspinhoBaixo5:
	li t0, 'T'				# P representa pedra no mapa
	bne t2, t0, checaChaveBaixo5			# Checa se tem esqueleto, se n�o segue normalmente		#Se n�o for espinho, checa chave
	la t1, colisao_temporaria
	li t2, 10
	mul t2, a6, t2
	add t2, t2, a3
...
moveDireita5:
	...
	add t1, t1, t2
	lb t2, 0(t1)
	bne t2, t0, checaEsqueletoDireita5
	addi a3, a3, -1
	j checaEspinhoDir5
...
checaEsqueletoDireita5:
	addi s10, s10, -1
	li t0, 'E'				# E representa esqueleto no mapa
	bne t2, t0, checaPedraDir5		# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver esqueleto e quadrado acima do esqueleto for "x", O Esqueleto morre
	lb t3, 1(t1)				#Armazena o quadrado a direita do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
...
checaPedraDir5:
	li t0, 'P'				# P representa pedra no mapa
	bne t2, t0, checaEspinhoDir5		# Checa se tem esqueleto, se n�o segue normalmente
	#Se tiver pedra e quadrado acima da pedra for "X" ou "P", a pedra n�o move
	lb t3, 1(t1)				#Armazena o quadrado acima do esqueleto
	li t4, 'X'				#Armazena  "x" em t4
	beq t3,t4, naoMovePedraDir4		# Se pedra estiver apoiada, n�o move
...
deixaEspinhoDir5:
	...
	lb t2, 0(t1)
	addi a3, a3, 1
	bne t2, t0, checaBauDireita5    #Se n�o for espinho, ele checa se � ba�
	addi a3, a3, -1
	la a4, espinho
	jal s9, spriteNotImm3
	addi a3, a3, 1
...
checaBauDireita5:	
	...
	li t0, 'B'				# Carrega "B"
	lb t3, 1(t1)				#Armazena o quadrado a direita do esqueleto
	bne t3, t0, DireitaLivre5	# Se o da direita n�o for B, anda 
	bne s5, zero, bauComChaveDireita5	 #Se tiver unm B e estiver com chave, abre o ba�	
						#Se tiver ba� mas n tiver chave:	       
	addi a3,a3, -1			        #recalcula posicao do her�i e contador de passos 
	addi s10,s10,1
	la a4, hero
...
fase_5UserChoice:
	...
	li t1,'s'					# Armazena caracter 's' em t1
	li t2, 10					# Armazena c?digo ascii da tecla enter em t2
	beq a0,t2,fase5_userChoose				# Se "enter for selecionado, salta o loop do menu
	beq a0,t1,fase_5ChangeChoice				# Se w for selecionado, muda sele??o
	beq a0,t0,fase_5ChangeChoice			# Se s for selecionado, muda sele??o
	j fase_5UserChoice 					# Se nem w, nem s, nem enter forem selecionadas, refaz o loop
fase_5ChangeChoice:
	jal changeFrame					# Muda tela
fase_5ChoicLoop:
...
fase_5UserChoice2:
	...
	li t1,'s'					# Armazena caracter 's' em t1
	li t2, 10					# Armazena c?digo ascii da tecla enter em t2
	beq a0,t2,fase5_userChoose2				# Se "enter for selecionado, salta o loop do menu
	beq a0,t1,fase_5ChangeChoice2				# Se w for selecionado, muda sele??o
	beq a0,t0,fase_5ChangeChoice2			# Se s for selecionado, muda sele??o
	j fase_5UserChoice2 					# Se nem w, nem s, nem enter forem selecionadas, refaz o loop
fase_5ChangeChoice2:
	jal changeFrame					# Muda tela
fase_5ChoicLoop2:
	j fase_5UserChoice2				#Reitera o loop
...
animacaoChute:
	...
	mv s0, a3		# salva o valor de a3, porque ser� usado pra volume
	li a0,45		# define a nota
	li a1,2500		# define a dura��o da nota em ms
